<template>
  <v-dialog v-model="modelOpen" width="800">
    <v-card>
      ...
      <v-card-text>
        <v-data-table item-value="id" :items="usages" :loading="loading">
          <template #headers>
            <tr>
          ...
          <template #item="{ item }">
            <tr>
              ...
              <td>
                <v-chip v-if="item.modified" color="warning" size="small">
      ...
      <v-card>
        ...
        <v-card-text>
          <v-form ref="editFormRef">
            ...
            <v-switch v-model="editForm.modified" :label="t('project.usage.modified')" />
            <v-textarea v-if="editForm.modified" v-model="editForm.modificationDescription"
              :label="t('project.usage.modificationDescription')" rows="3" />
            <v-select v-if="editForm.modified" v-model="editForm.supplierType" clearable item-title="title"
              item-value="value" :items="supplierTypeOptions" :label="t('project.usage.supplierType')" />
            <v-text-field v-if="editForm.modified" v-model="editForm.forkOriginUrl"
              :label="t('project.usage.forkOriginUrl')" :rules="[urlRule]" />
          </v-form>
        </v-card-text>
        <v-card-actions>
          <v-spacer />
