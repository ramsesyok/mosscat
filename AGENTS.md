...
**目的:** このファイルはリポジトリ内で *AI コーディングエージェント / LLM* が安全かつ一貫性を保って変更・生成を行うための最小限かつ機械が読みやすいガイドラインを提供する。詳細背景は `design.md` を参照し、本ファイルは “行動制約” と “主要コマンド” を列挙する。
